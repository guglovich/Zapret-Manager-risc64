...
chose_endpoint() {
while true; do
    echo -e "\n${MAGENTA}Выберите endpoint${NC}"

    i=1
    while IFS='|' read -r country ep; do
        printf "${CYAN}%d) ${GREEN}%-10s${MAGENTA}| ${CYAN}%s${NC}\n" "$i" "$country" "$ep"
        i=$((i+1))
    done <<EOF
$EP_LIST
EOF
    ...
    echo
    echo -en "${YELLOW}Выберите пункт (${NC}Enter = Россия${YELLOW}): ${NC}"
    read num
