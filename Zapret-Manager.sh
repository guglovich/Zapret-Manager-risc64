...
echo -e "${CYAN}Сохраняем текущие настройки...${NC}"; SECRET_IN_RS="$(sed -n 's/.*--secret[[:space:]]*\([0-9a-fA-F]\{32\}\).*/\1/p' "$INIT_PATH_RS")"; PORT_IN_RS="$(sed -n 's/.*--port[[:space:]]*\([0-9]\+\).*/\1/p' "$INIT_PATH_RS")"; PORT_IN_RS="${PORT_IN_RS:-2443}"; FAKETLS_DOMAIN="$(grep -oP '--listen-faketls-domain\s+\K[^[:space:]]+' "$INIT_PATH_RS" 2>/dev/null || echo "")"
...
install_TG_RS_FAKETLS() { echo -e "\n${MAGENTA}Установка TG WS Proxy Rust с Faketls Domain${NC}"; echo -en "${YELLOW}Введите SNI домен ${NC}(например: google.com): "; if [ -t 0 ]; then read FAKE_DOMAIN; else read FAKE_DOMAIN < /dev/tty; fi; [ -z "$FAKE_DOMAIN" ] && { echo -e "\n${RED}Домен не введён!${NC}\n"; PAUSE; return 1; }
...
if pgrep -f tg-ws-proxy-rs >/dev/null 2>&1 && [ -f "$BIN_PATH_RS" ] && [ -f "$INIT_PATH_RS" ]; then SECRET_IN_RS="$(sed -n 's/.*--secret[[:space:]]*\([0-9a-fA-F]\{32\}\).*/\1/p' "$INIT_PATH_RS")"; FAKETLS_IN_RS="$(grep -oP '--listen-faketls-domain\s+\K[^[:space:]]+' "$INIT_PATH_RS" 2>/dev/null || echo "")"
...
echo -e "${CYAN}4)${GREEN} Настроить внешний доступ ${CYAN}(Rust)${GREEN}"
echo -e "${CYAN}5)${GREEN} Обновить ${CYAN}TG WS Proxy Rust${GREEN} (сохранить настройки)"
echo -e "${CYAN}6)${GREEN} Установить ${CYAN}TG WS Proxy Rust${GREEN} с ${NC}Faketls Domain${GREEN} (SNI)"
...
3) if [ "$GO_ACTION" = "install" ] || [ "$GO_ACTION" = "update" ]; then install_update_TG_PKG; else remove_TG_PKG; fi ;; 4) ext_access_RS;; 5) update_TG_RS;; 6) install_TG_RS_FAKETLS;; *) break ;; esac; done; }
# ==========================================
