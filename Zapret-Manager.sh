...
# ==========================================
# УСТАНОВКА RUST
get_arch_RS() { if command -v opkg >/dev/null 2>&1; then ARCH="$(opkg print-architecture | awk '{print $2}' | tail -n1)"; elif command -v apk >/dev/null 2>&1; then ARCH="$(apk --print-arch 2>/dev/null)"; else ARCH="$(uname -m)"; fi; case "$ARCH" in aarch64*) echo "tg-ws-proxy-aarch64-unknown-linux-musl.tar.gz" ;; x86_64) echo "tg-ws-proxy-x86_64-unknown-linux-musl.tar.gz" ;; armv7*|armhf|armv7l) echo "tg-ws-proxy-armv7-unknown-linux-musleabihf.tar.gz" ;; mipsel_24kc|mipsel*) echo "tg-ws-proxy" ;; riscv64*) echo "tg-ws-proxy" ;; *) echo -e "\n${RED}Архитектура не поддерживается: ${NC}$ARCH\n"; PAUSE; return 1 ;; esac }
...
echo -e "${CYAN}Скачиваем и устанавливаем${NC} $ARCH_FILE_RS"; LATEST_TAG_RS="$(curl -Ls -o /dev/null -w '%{url_effective}' https://github.com/guglovich/tg-ws-proxy-rs-risc64/releases/latest | sed 's#.*/tag/##')"; [ -z "$LATEST_TAG_RS" ] && { echo -e "\n${RED}Не удалось получить версию${NC} TG WS Proxy Rust\n"; PAUSE; return 1; }
DOWNLOAD_URL_RS="https://github.com/guglovich/tg-ws-proxy-rs-risc64/releases/download/$LATEST_TAG_RS/$ARCH_FILE_RS"; if echo "$ARCH" | grep -q "mipsel"; then wget -q -U "Mozilla/5.0" -O "$BIN_PATH_RS" "$DOWNLOAD_URL_RS" >/dev/null 2>&1 || { echo -e "\n${RED}Ошибка скачивания${NC}\n"; PAUSE; return 1; }; elif echo "$ARCH" | grep -q "riscv64"; then wget -q -U "Mozilla/5.0" -O "$BIN_PATH_RS" "$DOWNLOAD_URL_RS" >/dev/null 2>&1 || { echo -e "\n${RED}Ошибка скачивания${NC}\n"; PAUSE; return 1; }; else wget -q -U "Mozilla/5.0" -O "$TMP_ARCHIVE_RS" "$DOWNLOAD_URL_RS" || { echo -e "\n${RED}Ошибка скачивания${NC}\n"; PAUSE; return 1; }
...
install_TG_RS_FAKETLS() { echo -e "\n${MAGENTA}Установка TG WS Proxy Rust с Faketls Domain${NC}"; echo -en "${YELLOW}Введите SNI домен ${NC}(например: google.com): "; read FAKE_DOMAIN </dev/tty; [ -z "$FAKE_DOMAIN" ] && { echo -e "\n${RED}Домен не введён!${NC}\n"; PAUSE; return 1; }
SECRET_FAKE="$(head -c16 /dev/urandom | hexdump -e '16/1 "%02x"')"; PORT_FAKE="2443"; ARCH_FILE_RS="$(get_arch_RS)" || { echo -e "\n${RED}Архитектура не поддерживается:${NC} $(uname -m)\n"; PAUSE; return 1; }; if ! command -v curl >/dev/null 2>&1; then echo -e "${CYAN}Устанавливаем ${NC}curl"; $UPDATE >/dev/null 2>&1 && $INSTALL curl >/dev/null 2>&1 || { echo -e "\n${RED}Ошибка установки curl${NC}\n"; PAUSE; return 1; }; fi
echo -e "${CYAN}Скачиваем и устанавливаем${NC} $ARCH_FILE_RS"; LATEST_TAG_RS="$(curl -Ls -o /dev/null -w '%{url_effective}' https://github.com/guglovich/tg-ws-proxy-rs-risc64/releases/latest | sed 's#.*/tag/##')"; [ -z "$LATEST_TAG_RS" ] && { echo -e "\n${RED}Не удалось получить версию${NC} TG WS Proxy Rust\n"; PAUSE; return 1; }
DOWNLOAD_URL_RS="https://github.com/guglovich/tg-ws-proxy-rs-risc64/releases/download/$LATEST_TAG_RS/$ARCH_FILE_RS"; if echo "$ARCH" | grep -q "mipsel"; then wget -q -U "Mozilla/5.0" -O "$BIN_PATH_RS" "$DOWNLOAD_URL_RS" >/dev/null 2>&1 || { echo -e "\n${RED}Ошибка скачивания${NC}\n"; PAUSE; return 1; }; elif echo "$ARCH" | grep -q "riscv64"; then wget -q -U "Mozilla/5.0" -O "$BIN_PATH_RS" "$DOWNLOAD_URL_RS" >/dev/null 2>&1 || { echo -e "\n${RED}Ошибка скачивания${NC}\n"; PAUSE; return 1; }; else wget -q -U "Mozilla/5.0" -O "$TMP_ARCHIVE_RS" "$DOWNLOAD_URL_RS" || { echo -e "\n${RED}Ошибка скачивания${NC}\n"; PAUSE; return 1; }; rm -rf "$TMP_DIR_RS"; mkdir -p "$TMP_DIR_RS"; tar -xzf "$TMP_ARCHIVE_RS" -C "$TMP_DIR_RS" || { echo -e "\n${RED}Ошибка распаковки${NC}\n"; PAUSE; return 1; }; mv "$TMP_DIR_RS"/tg-ws-proxy* "$BIN_PATH_RS"; rm -rf "$TMP_DIR_RS"; rm -rf "$TMP_ARCHIVE_RS"; fi; chmod +x "$BIN_PATH_RS"
printf '#!/bin/sh /etc/rc.common\nSTART=99\nUSE_PROCD=1\n\nstart_service() {\n    procd_open_instance\n    procd_set_param command /usr/bin/tg-ws-proxy-rs --host 0.0.0.0 --port %s --secret %s --listen-faketls-domain %s\n    procd_set_param respawn\n    procd_close_instance\n}\n' "$PORT_FAKE" "$SECRET_FAKE" "$FAKE_DOMAIN" > /etc/init.d/tg-ws-proxy-rs
chmod +x "$INIT_PATH_RS"; /etc/init.d/tg-ws-proxy-rs enable; /etc/init.d/tg-ws-proxy-rs start; if pidof tg-ws-proxy-rs >/dev/null 2>&1; then echo -e "${GREEN}Сервис ${NC}TG WS Proxy Rust${GREEN} запущен!${NC}\n"; echo -e "${YELLOW}Настройки:${NC}\n${YELLOW}Тип прокси:${NC} MTProto (Faketls)\n${YELLOW}Хост:${NC} $LAN_IP\n${YELLOW}Порт:${NC} $PORT_FAKE\n${YELLOW}Ключ:${NC} ee$SECRET_FAKE\n${YELLOW}SNI домен:${NC} $FAKE_DOMAIN\n${YELLOW}Ссылка для подключения:${NC}\ntg://proxy?server=$LAN_IP&port=$PORT_FAKE&secret=ee$SECRET_FAKE"; else echo -e "\n${RED}Сервис TG WS Proxy Rust не запущен!${NC}\n"; fi; PAUSE; }
# УСТАНОВКА GO
get_arch_GO() { if command -v opkg >/dev/null 2>&1; then ARCH="$(opkg print-architecture | awk '{print $2}' | tail -n1)"; elif command -v apk >/dev/null 2>&1; then ARCH="$(apk --print-arch 2>/dev/null)"; else ARCH="$(uname -m)"; fi; case "$ARCH" in aarch64*) echo "tg-ws-proxy-openwrt-aarch64";; armv7*|armhf|armv7l) echo "tg-ws-proxy-openwrt-armv7";; mipsel_24kc|mipsel*) echo "tg-ws-proxy-openwrt-mipsel_24kc";; mips_24kc|mips*) echo "tg-ws-proxy-openwrt-mips_24kc";; x86_64) echo "tg-ws-proxy-openwrt-x86_64";; riscv64*) echo "tg-ws-proxy-openwrt-riscv64";; *) echo "Неизвестная архитектура: $ARCH"; return 1;; esac }
...
$UPDATE >/dev/null 2>&1 && $INSTALL curl >/dev/null 2>&1 || { echo -e "\n${RED}Ошибка установки curl${NC}\n"; PAUSE ;return 1; }; fi; echo -e "${CYAN}Скачиваем и устанавливаем${NC} $ARCH_FILE_GO"; LATEST_TAG_GO="$(curl -Ls -o /dev/null -w '%{url_effective}' https://github.com/guglovich/-tg-ws-proxy-Manager-go-risc64/releases/latest | sed 's#.*/tag/##')"
[ -z "$LATEST_TAG_GO" ] && { echo -e "\n${RED}Не удалось получить версию${NC} TG WS Proxy Go\n"; PAUSE; return 1; }; DOWNLOAD_URL_GO="https://github.com/guglovich/-tg-ws-proxy-Manager-go-risc64/releases/download/$LATEST_TAG_GO/$ARCH_FILE_GO"; curl -L --fail -o "$BIN_PATH_GO" "$DOWNLOAD_URL_GO" >/dev/null 2>&1 || { echo -e "\n${RED}Ошибка скачивания${NC}\n"; PAUSE; return 1; }; chmod +x "$BIN_PATH_GO"
...
WAN_IP="$(curl -s --max-time 5 ifconfig.me 2>/dev/null || echo "не определён")"
if [ -z "$WAN_IP" ] || [ "$WAN_IP" = "не определён" ]; then WAN_IP="узнай на ifconfig.me"; fi
...
if [ "$ANSWER" = "y" ] || [ "$ANSWER" = "Y" ]; then
    if ! uci show firewall 2>/dev/null | grep -q "Allow-TG-Proxy"; then
        uci add firewall rule
        uci set firewall.@rule[-1].name='Allow-TG-Proxy'
        uci set firewall.@rule[-1].src='wan'
        uci set firewall.@rule[-1].dest_port="$PORT_IN_RS"
        uci set firewall.@rule[-1].proto='tcp'
        uci set firewall.@rule[-1].target='ACCEPT'
        uci commit firewall
        /etc/init.d/firewall restart
        echo -e "${GREEN}Порт $PORT_IN_RS открыт!${NC}"
    else
        echo -e "${YELLOW}Порт уже открыт!${NC}"
    fi
fi
echo -e "\n${GREEN}Перезапустить прокси с внешним доступом?${NC}"
echo -ne "  [y/N]: "; read ANSWER2
...
echo -e "${CYAN}3)${GREEN} Настроить внешний доступ ${CYAN}(Rust)${GREEN}"; echo -e "${CYAN}4)${GREEN} Обновить ${CYAN}TG WS Proxy Rust${GREEN} (сохранить настройки)"
echo -e "${CYAN}5)${GREEN} Установить ${CYAN}TG WS Proxy Rust${GREEN} с ${NC}Faketls Domain${GREEN} (SNI)"
echo -e "${CYAN}Enter) ${GREEN}Выход в главное меню${NC}\n"; echo -en "${YELLOW}Выберите пункт: ${NC}"; read choice; case "$choice" in 1) if [ -f "$BIN_PATH_GO" ] && [ -f "$INIT_PATH_GO" ]; then delete_TG_GO; else install_TG_GO; fi;; 2) if [ -f "$BIN_PATH_RS" ] && [ -f "$INIT_PATH_RS" ]; then delete_TG_RS; else install_TG_RS; fi;; 3) ext_access_RS;; 4) update_TG_RS;; 5) install_TG_RS_FAKETLS;; *) break ;; esac; done; }
# ==========================================

# Launcher
echo "#!/bin/sh" > /usr/bin/zms
echo "sh /tmp/Zapret-Manager.sh" >> /usr/bin/zms
chmod +x /usr/bin/zms
# Главное меню
# ==========================================
